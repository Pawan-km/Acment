...
const Mentor = require("../models/MentorDB");
...
exports.users_post_register = async (req, res) => {
  ...
  console.log("Creting user......", username, email, password, exam,avator)
...
exports.users_post_login = async (req, res) => {
  ...
  try {
    ...
    console.log(req.body);
    const user = await students.findOne({ email: email }).lean();
...
exports.users_post_changePassword = async (req, res) => {
  ...
  try {
    ...
    const user = await students.findOne({ _id: _id }).lean();
    if (!user) {
      return res.status(400).json({ message: "Invalid email/password" });
    }
...
exports.follow_mentor = async (req, res) => {
  try {
    ...
    updateUser = await students.findOneAndUpdate(
      { _id: req.userData.id },
      {
        $addToSet: {
          following: mentorId,
...
exports.followings = async (req, res) => {
  ...
  try {
    userData = await students.findOne({ _id: req.userData.id }).populate(
      "following",
      "username email"
    );
...
exports.user_patch_Data = async (req, res) => {
  try {
    const updateData = await students.findOneAndUpdate(
      { _id: req.userData.id },
      req.body,
      {
        new: true,
...
exports.users_post_profile = async (req, res, next) => {
  try {
    ...
    updateProfile = await students.findByIdAndUpdate(
      { _id: req.userData.id },
      { profile: path },
      { new: true }
    );
